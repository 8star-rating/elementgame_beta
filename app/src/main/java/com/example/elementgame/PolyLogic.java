...
public class PolyLogic {
    ...
    public PolyLogic(Context context){
        readCSV(context);
    }
    ...
    private void readCSV(Context context){
        try {
            BufferedReader br = new BufferedReader(
                    new InputStreamReader(context.getAssets().open("polyatomics.csv")));
            br.readLine();
            String line;
    ...
    public String getNextIon(){
        int index = random.nextInt(elements.size());
        String [] entry = elements.get(index);
        currentSymbol = entry[1];
        currentAnswer = entry[0].trim();
        Qcount++;
